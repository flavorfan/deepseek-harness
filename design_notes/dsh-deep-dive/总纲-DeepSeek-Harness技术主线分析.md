...
> 本文是整份深度解析的导论与主线索引。它先给出一条能贯穿全部章节的"主线论断"——也就是理解这套系统的那把总钥匙——再展开阅读路径与主线机制；后续各章，都是在这条主线上填入细节。
>
...
> 若要用一句更短的口号记住它：**「万物皆服务，万物皆可撤销」**——前半句是"一切皆插件"的服务化表达（能力都做成经 `ctx` 解析的 service），后半句是可逆 effect（revertible effect）的通俗说法（卸载能结构性回收）。这是对底座范式的凝练口号，不是被证明的定理；其严格边界见 [Part IV 第 22 章 §1.7](Part%20IV%20Foundational%20Paper/22-A-Programming-Paradigm-for-Spatiotemporal-Composability.md)。
...
- **Part V Cordis 深度调研**（Ch24–29）：对底座框架 cordiverse/cordis 单独跑一遍源码深度解析，并厘清 Cordis · Koishi · dsh 三者关系。对 Cordis 本体的完整逐机制解析另见姊妹报告《[Cordis 源码深度解析](https://github.com/xiaonancs/cordis-source-analysis/blob/main/README.md)》（基线 rc.9 / `00278924`；本 Part V 为更早的 rc.7/rc.8 快照，行号不可互引）。
