...
两次 tsdown 都接收由 `vendor/*/package.json`、`packages/*/*/package.json` 与 `apps/cli/package.json` 发现出的同一组完整 workspace，且都只打包 `lib/types` 中由对应 tsc 阶段发射的 JavaScript。根配置不扫描 Client 产物、不按包名分类，也不向 tsdown 传维护式 filter；各包的本地配置根据 `DSH_BUILD_FACE` 返回当前阶段的入口，缺少 manifest 的已删除 package 残留目录不会进入构建。普通 Client 插件在 Client 阶段一起生成 Node loader 入口与 browser bundle。
