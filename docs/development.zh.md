...
两次 tsdown 都使用由 `vendor/*/package.json`、`packages/*/*/package.json` 与 `apps/cli/package.json` 发现出的同一组完整 workspace，不扫描构建产物来发现 Client 包，也不维护 Host/Client 包过滤表；缺少 manifest 的已删除 package 残留目录不会进入构建。包内 tsdown 配置根据 `DSH_BUILD_FACE` 决定当前阶段的入口：普通 Client 插件在 Client 阶段同时生成 Node loader 与 browser bundle；`api-remotes` 通过 `hostPhase: true` 提前生成 Host 入口，再在 Client 阶段只生成 browser bundle。tsdown 只消费 `lib/types` 中由前置 tsc 发射的 JavaScript。
