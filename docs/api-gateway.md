...
Both tsdown passes receive the same complete workspace discovered from `vendor/*/package.json`, `packages/*/*/package.json`, and `apps/cli/package.json`, and bundle only JavaScript emitted to `lib/types` by the corresponding tsc phase. The root config does not scan Client artifacts, classify package names, or pass a maintained filter to tsdown; package-local configs return entries for the current phase based on `DSH_BUILD_FACE`, and deleted-package residue directories without a manifest do not join the build. An ordinary Client plugin produces both its Node loader entry and browser bundle during the Client phase.
